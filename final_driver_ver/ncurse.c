...
int main() {
    ...
    while (1) {
        ...
        if (!dropdown) {
            ...
            case KEY_LEFT:
                selected_room =
                    (selected_room - 1 + MENU_ROOM_COUNT) % MENU_ROOM_COUNT;
                break;
            case KEY_RIGHT:
                selected_room =
                    (selected_room + 1) % MENU_ROOM_COUNT;
                break;
            case KEY_DOWN:
            case '\n':
                dropdown = 1;
            ...
            case '\n':
                ...
                mvprintw(ROW_OFFSET+2, 2, "Room   : %s", room);
                mvprintw(ROW_OFFSET+3, 2, "Action : %s", action);
                ...
                if (strcmp(action, "reserve") == 0) {
                    ...
                    mvprintw(ROW_OFFSET+7, 2, "Reserved by user: %s", user_id);
                } else {
                    mvprintw(ROW_OFFSET+5, 2, "Execute %s ...", action);
                }
